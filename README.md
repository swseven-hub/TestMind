...
功能用例会被要求覆盖正向和逆向场景，而不是只生成 happy path。
...
为了避免只生成正向用例，TestMind 增加了模块级质量检查：

- 功能用例必须是数量最多的分类。
- 每个显式测试点尽量生成正向和逆向功能用例。
- 如果某个模块的逆向功能用例不足，后端会触发补充生成。
- 补充方向是通用的，不绑定某个行业：
  - 身份 / 访问问题
  - 表单 / 字段问题
  - 流程 / 状态问题
...
可以。没有 API Key 时会使用本地规则生成演示结果，但质量和覆盖度不如大模型生成。
