...
| 能力 | 说明 |
| --- | --- |
| 智能体工作台 | 首页左侧可切换需求评审、用例生成和发布风险智能体 |
| 需求评审 | 上传 PRD PDF，输出疑点、边界、异常、权限、风险、检查清单和下一步 |
| 发布风险 | 粘贴发布材料，输出回归范围、P0/P1 风险、冒烟清单和上线检查项 |
| 评审过程可视化 | 需求评审和发布风险分析支持过程弹窗，展示当前阶段、运行时间、模型实时输出和停止按钮 |
...
需求评审智能体只支持 PRD PDF 上传；发布风险智能体保留文本材料输入。两者都支持无 API Key 的本地规则兜底；配置 DeepSeek、阿里云百炼、OpenAI 或 Velotric Key 后会调用模型生成结构化报告。
