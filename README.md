...
需求评审智能体只支持 PRD PDF 上传；发布风险、变更影响和 Bug 根因智能体支持文本输入、主材料文件和依据文档。长日志或长协议文档会自动保留开头、关键错误/接口/diff/协议片段和结尾，不再因为超过 80000 字符直接失败。四个分析类智能体都支持无 API Key 的本地规则兜底；配置 DeepSeek、阿里云百炼、OpenAI 或 Velotric Key 后会调用模型生成结构化报告。
