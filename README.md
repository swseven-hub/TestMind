...
> AI testing agent workbench for a solo QA workflow. It keeps the original PRD-to-test-case generator, and adds lightweight agents for requirement review and release risk analysis.
...
| 能力 | 说明 |
| --- | --- |
| 智能体工作台 | 首页左侧可切换需求评审、用例生成、发布风险和变更影响智能体 |
...
| 评审过程可视化 | 需求评审、发布风险和变更影响分析支持过程弹窗，展示当前阶段、运行时间、模型实时输出和停止按钮 |
...
需求评审智能体只支持 PRD PDF 上传；发布风险和变更影响智能体保留文本材料输入。三个分析类智能体都支持无 API Key 的本地规则兜底；配置 DeepSeek、阿里云百炼、OpenAI 或 Velotric Key 后会调用模型生成结构化报告。
