...
TestMind 是一个本地运行的 Next.js 应用，用来把 PRD PDF 转成中文、可执行的测试用例。核心流程是：

1. 上传 PRD PDF。
2. 选择 AI 供应商、模型，以及可选的推理等级或生成模式。
3. 后端流式返回生成进度：解析 PDF、生成覆盖蓝图、按模块生成用例、补齐覆盖缺口、去重编号并保存运行记录。
4. 前端支持查看、筛选、搜索测试用例，查看覆盖蓝图和运行统计，导出 Excel，或回看历史记录。
...
- `src/app/page.tsx`：首页客户端 UI。负责上传、供应商/API Key localStorage、模型配置、流式 NDJSON 消费、进度弹窗、演示数据、筛选、搜索、用例卡片、覆盖蓝图、统计和 Excel 导出。
...
- `useClientReady` 控制 API Key 区域：服务端先渲染骨架屏，客户端 ready 后显示真实本地配置。
...
- 如果 API Key 区域一直是浅色骨架块，优先检查 `useClientReady`、hydration 和 Next dev-origin 警告。使用 `127.0.0.1` 时，`allowedDevOrigins` 需要包含 `127.0.0.1`。
