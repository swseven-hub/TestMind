...
TestMind 是一个本地运行的 Next.js AI 测试智能体工作台，用来支撑唯一测试在需求分析、用例生成、发布风险、变更影响和 Bug 根因分析中的高频工作。核心流程是：
...
- `src/app/analysis/[id]/page.tsx`：需求分析详情页。负责展示需求分析智能体的全部模块、测试点、优先级筛选、搜索、分页、检查清单和下一步动作。
- `src/app/cases/[id]/page.tsx`：测试点详情页。负责按运行记录或当前会话结果展示全部测试点、模块目录、类型筛选、搜索、分页和 Excel 导出。
...
- `src/app/api/agents/analyze/stream/route.ts`：分析类智能体主接口，支持需求分析、发布风险、变更影响和 Bug 根因分析的过程流。
...
- 首页执行面板不显示模型设置；模型、密钥、推理等级和 Velotric 网关统一维护在 `/settings`，并对所有智能体全局生效。
...
- `useClientReady` 控制首页运行按钮：服务端先用默认配置，客户端 ready 后读取真实本地配置。
...
- 前端改动后运行 `npm run dev` 并在浏览器验证：
  - 首页顶部设置入口能跳转 `/settings`。
  - 设置页能切换供应商、模型和推理等级，并能返回工作台。
  - Bug 根因智能体不显示文本框，上传主材料文件后能运行。
  - 快速体验能加载 36 条演示用例，筛选和搜索可用。
  - 需求分析完成后首页只展示概览，点击“查看全部测试点”进入 `/analysis/[id]`，详情页目录、搜索、优先级筛选和分页可用。
  - 生成后首页只展示报告概览，点击“查看全部测试点”进入 `/cases/[id]`，详情页目录、搜索、筛选和分页可用。
  - 演示结果或历史记录可以导出 Excel。
