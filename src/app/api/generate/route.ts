...
function getPrompt(text: string, fileName: string) {
  ...
  const systemPrompt =
    "你是资深测试架构师。请从任意行业、任意产品形态的 PRD 中先识别所有可测试模块，再按模块生成完整测试用例。每条用例必须标注 category，覆盖功能、边界、异常、权限、性能。输出必须是中文、可执行、无 Markdown，且不得引入 PRD 无依据的行业假设。";
...
字段要求：
1. 先识别 PRD 中所有明确或隐含的可测试模块、子模块、流程、角色、对象、字段规则、状态、权限、集成和数据约束。
2. module 必须填写具体业务模块名称，不要填写“需求点”“系统”“页面”等泛化名称。
3. 功能用例必须最多，并覆盖正向和逆向功能路径；不要只生成 happy path。
4. 每个模块都应尽量覆盖功能、边界、异常、权限、性能；若模块不适合性能或权限，也要给出可验证的相关场景。
5. category 只能是 功能、边界、异常、权限、性能；priority 只能是 P0、P1、P2。
6. cases 总数通常生成 80 到 180 条；PRD 很简单时也不少于 40 条。`;
